...
def cost(graph, data, centers):
    ...
    for i in range(n_samples):
        for j in range(n_clusters):
            distances[i, j] = custom_distance(graph, data[i], centers[j])
    ...
    for i, x in enumerate(data):
        ...
        total_cost += custom_distance(graph, x, center)
    return total_cost
...
# C_0 = np.load("D:\\Users\\zmche\\Desktop\\pythonProject\\graph\\Initial\\C_0_DP_HST.npy")  # DP_HST方法
