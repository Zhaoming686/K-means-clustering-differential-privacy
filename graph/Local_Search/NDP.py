...
class CustomKMeans(KMeans):
    ...
    def _eucledean_dist(self, X, centers):
        ...
        for i in range(n_samples):
            for j in range(n_clusters):
                distances[i, j] = custom_distance(self.graph, X[i], centers[j])
    ...
    def compute_total_cost(self, X):
        ...
        for i, x in enumerate(X):
            ...
            total_cost += custom_distance(self.graph, x, center)
        return total_cost
...
# 选择初始化方法
C_0 = None  # 随机初始化
# C_0 = np.load("D:\\Users\\zmche\\Desktop\\pythonProject\\graph\\Initial\\C_0_Kmeans_plus_plus.npy")  # kmeans++的方法
# C_0 = np.load("D:\\Users\\zmche\\Desktop\\pythonProject\\graph\\Initial\\C_0_HST.npy")  # HST方法
