...
def build_two_HST(T, graph, delta, currentNode, L):
    ...
    while len(U) > 0:  # Process nodes until U is empty
        v = U[0]  # 获取第一个元素作为初始点
        C_v = [v]
        indexToRemove = []

        for index, u in enumerate(U[1:], start=1):  # 从第二个元素开始遍历

            d_uv = euclidean_distance(u, v)
...
def save_C0_as_numpy(C_0, filename="C_0_DP_HST.npy"):
    # 提取节点信息
...
# 定义函数，动态生成 Laplace 噪声
def generate_laplace_noise(L, hv, epsilon, size=1):
    # 计算尺度参数 b
    b = 2 * (L - hv) / epsilon
    # 生成 Laplace 噪声
...
def getDelta(U):
    ...
    # 抽取的点数：根号n
    num_samples = math.floor(math.sqrt(n))  # 取整方式可调整
    if num_samples < 2:  # 至少需要 2 个点
        return 0

        # 随机抽取 num_samples 个点
    sampled_indices = np.random.choice(n, num_samples, replace=False)  # 随机选索引
    sampled_points = U[sampled_indices]  # 根据索引取数据

    # 计算采样点之间的最大欧氏距离
    for i in range(num_samples):
        for j in range(i + 1, num_samples):  # 避免重复计算
            d_uv = euclidean_distance(sampled_points[i], sampled_points[j])
            if d_uv > delta:
                delta = d_uv
...
def initialHST(T):
    ...
    for level in T.L_nodes:
        for node in T.L_nodes[level]:
            ...
            L = 2
            hv = 2 - level
            laplace_noise = generate_laplace_noise(L, hv, epsilon)  # 随时生成 Laplace 噪声
            laplace_noise_float = float(laplace_noise[0])  # 确保结果是浮动类型
            scores[node] = N_v * (2 ** hv)
